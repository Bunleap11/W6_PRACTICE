...
export async function getArticlesByJournalistId(req, res) {
  try {
    const articles = await pool.query(
      `SELECT articles.*, journalists.name as journalistName 
        FROM articles JOIN journalists ON articles.journalistsId = journalists.id 
        WHERE articles.journalistsId = ?`,
      [req.params.id]
    );
    res.json(articles[0]);
  } catch (error) {
...
export async function getAllCategory(req, res) {
  ...
  try {
    const categories = await articleRepository.getAllCategory();
    console.log(categories);
    res.json(categories);
  } catch (error) {
    console.error("Error fetching categories");
    res.status(500).json({ message: "fail to fetch categories" });
  }
}
export async function getArticlesByCategoryId(req, res) {
  try {
    ...
    res.status(500).json({ message: "Fail to fetch articles" });
  }
}
